...
fn test_write(request: &Request) -> String {
    match *request {
        ...
        Request::AddColumn(ref req) => format!(
            "AddColumn {{column_name: {:?}, column_type: {:?}}}",
            req.column_name, req.column_type
        ),
        Request::Insert(ref req) => {
            let r = Regex::new(r"(\d+): (\w+).\[([^\]]+)\].").unwrap();
            let pre = format!(
                "Insert {{source: {}, timestamps: #{}, columns: [",
                req.source(),
                req.timestamps().len()
            );
            let columns = req.columns().iter().fold("".to_owned(), |acc, column| {
                ...
                let lala = r
                    .captures_iter(&col_string)
                    .fold("".to_owned(), |acc2, cap| {
                        let col = format!(
                            "{}: {} #{}",
                            &cap[1],
                            &cap[2],
        ...
        Request::Scan(ref req) => format!(
            "Scan {{min_ts: {}, max_ts: {}, partition_ids: #{}, projection: {:?}, \
                      filters: {:?}}}",
            req.min_ts,
            req.max_ts,

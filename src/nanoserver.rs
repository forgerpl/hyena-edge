...
use nanomsg_multi_server::proto::{PeerError, PeerReply, PeerRequest};
use nanomsg_multi_server::{MultiServer, MultiServerFutures};

use hyena_api::{run_request, Reply, Request};
...
fn process_message(msg: Vec<u8>, catalog: &mut Catalog) -> Vec<u8> {
    ...
    serialize(&reply).expect("Reply serialization error")
}
...
pub fn run(matches: clap::ArgMatches) {
    ...
    let session_ipc_path = PathBuf::from(
        matches
            .value_of("session_ipc_path")
        ...
        thread::Builder::new()
            .spawn(move || {
                let catalog = Rc::new(RefCell::new(
                    Catalog::open_or_create(dir).expect("Unable to initialize catalog"),
                ));
                ...
                cat_reader
                    ...
                    .for_each(|msg: (_, _, _, UnboundedSender<_>)| {
                        ...
                        output_channel
                            ...
                            .unwrap_or_else(|_| {
                                error!("session was already closed before reply could be sent")
                            });
                    });
            })
                ...
                reader
                    .for_each(move |msg| {
                        ...
                        match msg {
                            ...
                            KeepAlive => {
                                debug!("[{connid}] Keepalive Request", connid = connid);
